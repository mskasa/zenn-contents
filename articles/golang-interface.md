...
インタフェースの特長は、コードの柔軟性、拡張性、再利用性の向上です。
この特長は、インタフェースを用いることで実現する、多態性（ポリモーフィズム）や依存性の注入（DI）によるものです。

### 多態性（ポリモーフィズム）

ポリモーフィズムとは、異なる型が同じインタフェースを通して異なる動作をすることです。

以下の例では、異なる型である`Circle`と`Rectangle`が同じインタフェース`Shape`を通して、同一のシグネチャであるメソッド`Area() float64`で異なる動作をします。
図形の種類に関わらず、同じ方法（シグネチャ）で、そのオブジェクトを操作できるということです（= 柔軟性）。
