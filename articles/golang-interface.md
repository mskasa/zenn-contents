...
最後に、インタフェースの使いどころについて解説します。
これまで、インタフェースのメリットを紹介してきました。では、取り敢えずインタフェースを使いまくればよいのでしょうか？もちろんそんなことはありません。

インタフェースの最大のデメリットは実装が複雑化することです。過剰に複雑な実装をするぐらいなら、インタフェースを使わずにコードを書く方が好まれるでしょう。Go言語では、インタフェースを使わずにコードを書いておき、必要だと分かった段階で利用する形に書き換えるというアプローチが合理的です。なぜなら、Go言語では、インタフェースが暗黙的に実装されるという設計原則が取り入れられているからです。

今更ですが、Go言語では、型が特定のインタフェースを実装しているかどうかは、その型がインタフェースで定義された全てのメソッドを持っているかどうかに基づいて自動的に判断されます。これに対して、例えば Java言語では、クラスが特定のインターフェースを実装する場合、そのインターフェースを明示的に`implements`キーワードを使って宣言する必要があります。Go言語では、そのような明示的な宣言が不要であるため、後からインタフェースを追加する際の既存コードへの影響が少ないのです。

まとめると、Go言語では、インタフェースを中心に設計するのではなく、開発過程でインタフェースを発見し、必要に応じて適用するアプローチが合理的です。このアプローチでは、開発者が事前に定義されたインタフェースに縛られることなく、実際のニーズやパターンに応じて柔軟にインタフェースを定義できます。
