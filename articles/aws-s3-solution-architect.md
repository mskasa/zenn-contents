...
:::message
バックアップ絡みで知っておく必要があります。例えば、バックアップ保持期間が 1ヶ月の場合、Glacierは余計なコストが発生するため、最適ではありません。
あと、言うまでもないですが、最小ストレージ期間があるものは一時ファイル保管場所としては不適切です。
:::
### Intelligent-Tiering
...
リクエストする側は、課金されることを了承している旨を示すため、リクエストのヘッダーに`x-amz-request-payer`を含める必要があります。
...
:::message
SSE-S3暗号化では、オブジェクトデータのみが暗号化されます（オブジェクトのメタデータは対象外）。
:::
...
:::message
オンプレミスや別リージョンからゲートウェイエンドポイントへのアクセスは、ゲートウェイエンドポイントと同一のVPCにプロキシとして立てたEC2を経由することで実現できます。
ただし、EC2そのものの稼働コストや、可用性の面を考えると良い選択ではありません。素直にシンプルかつ管理不要なインターフェイスエンドポイントを利用した方がよいでしょう。
:::
